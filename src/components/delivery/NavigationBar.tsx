...
interface NavigationBarProps {
  ...
  speed: number;
  followMode: boolean;
  gpsStatus: "unavailable" | "acquiring" | "active" | "stale" | "inaccurate" | "error";
...
export function NavigationBar({
  ...
  const hasRoadRoute = route?.status === "success" && !isStale;
  const routeStatusLabel = !route ? "Waiting for road route" : null;
  ...
  return (
    <div className="flex flex-col gap-0">
      {externalNavUrl && !isStale && (
        <a
          className="block text-center text-sm underline py-2"
          href={externalNavUrl}
          target="_blank"
          rel="noreferrer"
        >
          Open road directions in OpenStreetMap (car)
        </a>
      )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-right shrink-0">
              <p className="text-lg font-bold font-mono text-white">
                {distanceToNextStep !== null ? formatDistanceShort(distanceToNextStep) : "—"}
              </p>
            </div>
          </div>
        </div>
      ...
      {isArrived && (
        <div className="rounded-2xl bg-emerald-600 px-4 py-4 shadow-xl ring-1 ring-white/10 text-center">
          ...
          <p className="text-base font-bold text-white">
            {arrivalZone === "at_vendor"
              ? "You've arrived at the shop!"
              : "You've arrived at the customer!"}
          </p>
          <p className="mt-1 text-xs text-white/80 font-medium truncate">{destinationLabel}</p>
        </div>
      )}

      {/* ── Near destination approach ── */}
      {isNear && hasRoadRoute && !isArrived && (
        <div className="rounded-xl bg-amber-500/95 px-3 py-2 text-center shadow-lg mt-2">
          <p className="text-sm font-semibold text-white">
            📍 Almost there! {distanceToDestM !== null ? formatDistanceShort(distanceToDestM) : "—"}{" "}
            away
          </p>
        </div>
      )}
      ...
      <div className="mt-2 rounded-2xl bg-slate-900/95 shadow-xl ring-1 ring-white/10 backdrop-blur-lg overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-mono text-lg font-bold text-white">
                {!route || isStale || distanceToDestM === null
                  ? "—"
                  : isRerouting
                    ? "..."
                    : formatDistanceShort(distanceToDestM)}
              </p>
            </div>
            {/* Speed */}
            <div>
              <p className="font-mono text-xs text-slate-500">SPEED</p>
              <p className="font-mono text-sm font-bold text-slate-300">{speedKmh} km/h</p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between border-t border-white/5 px-4 py-1.5 text-[10px] font-mono">
          <span
            className={`flex items-center gap-1 ${gpsStatus === "error" ? "text-red-400" : gpsStatus === "stale" || gpsStatus === "inaccurate" ? "text-amber-400" : "text-emerald-400"}`}
          >
            <span className="relative flex h-1.5 w-1.5">
              {gpsStatus !== "stale" && gpsStatus !== "error" && (
