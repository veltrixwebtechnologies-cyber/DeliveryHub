        ...
        <li key={item.to} className={compact ? "min-w-0 flex-1" : undefined}>
          <Link
            ...
            className={cn(
              ...
              compact
                ? "flex-col gap-1 rounded-xl px-2 py-2 text-[11px]"
          ...
          >
            <span className={cn("grid place-items-center", compact ? "h-5 w-5" : "h-8 w-8 rounded-xl bg-muted")}>{item.icon}</span>
    ...
    <div className="min-h-screen bg-background lg:grid lg:grid-cols-[248px_1fr]">
      <aside className="hidden border-r border-border/70 bg-card/70 px-4 py-5 lg:block">
        <Link to="/" className="group flex items-center gap-3 px-2">
          ...
          <span>
            <span className="block text-sm font-semibold tracking-tight text-foreground">LocalShoree</span>
            <span className="block text-xs text-muted-foreground">Delivery Partner Hub</span>
          </span>
        </Link>
        <div className="mt-10 px-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Workspace</div>
        ...
        <div className="mt-10 rounded-2xl border border-border bg-secondary/60 p-4">
          ...
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">Support is available while you are on a delivery.</p>
          <a className="mt-3 inline-flex text-xs font-semibold text-primary hover:underline" href="mailto:support@localshoree.com">Contact support</a>
        </div>
      </aside>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-40 border-b border-border/70 bg-card/85 shadow-soft backdrop-blur-xl">
          <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3 lg:max-w-none lg:px-8">
            <Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setMenuOpen((v) => !v)} aria-label="Open navigation">
        ...
        {menuOpen ? <div className="fixed inset-0 z-30 bg-foreground/20 backdrop-blur-sm lg:hidden" onClick={() => setMenuOpen(false)} /> : null}
        {menuOpen ? <div className="fixed left-0 top-[61px] z-40 w-[min(86vw,320px)] border-r border-border bg-card p-4 shadow-elegant lg:hidden"><nav>{navigation()}</nav></div> : null}
        <main className="animate-fade-up mx-auto max-w-6xl px-4 py-6 pb-24 lg:max-w-none lg:px-8">{children}</main>
        <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border/80 bg-card/90 px-2 py-2 shadow-elegant backdrop-blur-xl lg:hidden">{navigation(true)}</nav>
      </div>
    </div>
  );
}
