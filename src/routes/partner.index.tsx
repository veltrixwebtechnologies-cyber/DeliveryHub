...
function PartnerDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: todays }, { data: weeks }, { data: act }, { data: assignmentStats }] =
        await Promise.all([
          db
          ...
          db
            .from("delivery_assignments")
            .select("status,distance_km,accepted_at,delivered_at,created_at")
            .eq("partner_id", partner.id),
        ]);
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[28px] bg-gradient-primary p-6 text-primary-foreground shadow-glow sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-white/20 bg-white/10 px-4 py-3 backdrop-blur">
            <span
              className={`h-2.5 w-2.5 rounded-full ${isBusy ? "bg-amber-300" : partner.availability === "online" ? "bg-emerald-300 shadow-[0_0_0_5px_rgba(110,231,183,0.15)]" : "bg-white/50"}`}
            />
            <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={INR(today.earnings)}
          hint={`${today.deliveries} completed today`}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_0.9fr]">
        <Card className="overflow-hidden border-border/80 shadow-soft">
          <CardHeader className="flex-row items-start justify-between space-y-0 border-b border-border/70 bg-secondary/30">
            <div>
              <CardTitle className="text-base">Your route today</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Stay focused on the next handoff.
              </p>
            </div>
          ...
          <CardContent className="p-5">
            {active.length === 0 ? (
              <EmptyState
                ...
                description="Accepted requests will show the pickup, route and drop details here."
                action={
                  <Button asChild size="sm">
                    <Link to="/partner/deliveries">Open deliveries</Link>
                  </Button>
                }
              />
            ) : (
        ...
        <Card className="border-border/80 shadow-soft">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">Small improvements add up.</p>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-border pt-4">
              <MiniMetric
                label="Working hours"
                value={`${Math.floor(performance.workingMinutes / 60)}h ${String(performance.workingMinutes % 60).padStart(2, "0")}m`}
              />
              <MiniMetric
                label="Distance covered"
                value={`${performance.distanceKm.toFixed(1)} km`}
              />
            </div>
          </CardContent>
        </Card>
...
function QuickAction({
  ...
  return (
    <Link
      to={to}
      className="group rounded-2xl border border-border bg-card p-4 shadow-soft transition-smooth hover:-translate-y-1 hover:shadow-elegant"
    >
      <span className="grid h-10 w-10 place-items-center rounded-xl bg-secondary text-secondary-foreground transition-smooth group-hover:bg-primary group-hover:text-primary-foreground">
        {icon}
      </span>
